...
import {
  ...
  getNextMonth,
  createDateKey,
} from "@/app/lib/date/date-helpers";
...
export const useCalendar = () => {
  ...
  const fetchUserAccounts = useCallback(async () => {
    ...
    try {
      ...
      setAccounts((response.data?.items || []) as Account[]);
    } catch (error) {
  ...
  const processTransactionsByDay = useCallback(
    (transactions: any[], currentDate: Date) => {
  ...
  const fetchMonthTransactions = useCallback(
    async (date: Date, accountId: string | "all" = "all") => {
