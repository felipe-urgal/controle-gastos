// app/api/transaction/import/route.ts
import { NextRequest } from "next/server";
import { CsvProcessor } from '@/app/lib/csv/csvProcessor';
import { ImportResponse } from '@/app/lib/csv/importResponse';
import { TransactionImportService } from '@/app/services/transactionImportService';
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        const normalizedHeader = CsvProcessor.normalizeHeader(
          header, 
          TransactionImportService.CSV_OPTIONS.columnMappings
        );
        record[normalizedHeader] = values[index];
      });

      const result = await TransactionImportService.processTransactionRecord(record, userId);
