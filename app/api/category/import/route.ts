...
import { CategoryImportService } from '@/app/services/categoryImportService';
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!CategoryImportService.validateRequiredColumns(headers)) {
      return ImportResponse.createErrorResponse(
        'Coluna obrigatória faltando. Use: name ou nome',
        400
      );
    }
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      headers.forEach((header, index) => {
        const normalizedHeader = CsvProcessor.normalizeHeader(
          header, 
          CategoryImportService.CSV_OPTIONS.columnMappings
        );
        record[normalizedHeader] = values[index];
      });

      // Usar o serviço para processar o registro
      const result = await CategoryImportService.processCategoryRecord(record, userId);
